...
# --- Базові компоненти CAPEX для МБО та Сортувальної станції (€/т/рік) ---
base_capex_components = {
    "MBO": {"mechanical": 150, "biological": 300, "storage": 50, "civil_works": 100, "electrical": 40, "engineering": 30, "contingency": 0.1},
    "Sorting": {"sorter": 120, "storage": 40, "civil_works": 80, "electrical": 30, "engineering": 25, "contingency": 0.1}
}
...
def calc_capex(annual_tonnage, plant_type):
    components = base_capex_components[plant_type]
    capex_sum = annual_tonnage * sum([v for k,v in components.items() if k!="contingency"])
    capex_total = capex_sum * (1 + components["contingency"])
    return {
...
def compute_range(weekly_range):
    ...
    for plant_type in ["MBO","Sorting"]:
        capex = calc_capex(annual_tonnage, plant_type)
        ...
        print(f"\n--- Розрахунок для {plant_type} діапазону {weekly_range[0]} – {weekly_range[1] if weekly_range[1]!=math.inf else '∞'} т/тиждень ---")
...
# --- Обрахунок усіх кластерів та збереження в CSV ---
def compute_all_clusters_to_file():
    ...
    for idx, row in df.iterrows():
        weekly_tonnage = row['waste_tonnes_week']
        annual_tonnage = weekly_tonnage*52
        ...
        for plant_type in ["MBO","Sorting"]:
            ...
            total_CAPEXC += capex["Conservative"]
            total_CAPEXB += capex["Base"]
            total_CAPEXO += capex["Optimistic"]
        ...
        results.append({
            ...
            "CAPEXC": round(total_CAPEXC,2),
            "CAPEXB": round(total_CAPEXB,2),
            "CAPEXO": round(total_CAPEXO,2)
        })

    out_df = pd.DataFrame(results)
    out_df.to_csv("cost_sort.csv", index=False)
    print("\nРозрахунки завершено. Файл 'cost_sort.csv' створено.")
    pause()
